<script lang="ts">
  ...
  let currentUser = $state<{ login: string; id: number; avatar_url: string } | null>(null);
  ...
  async function loadRepositories(): Promise<void> {
    try {
      ...
      for (const org of organizations) {
        try {
          const repos = await extension.fetchRepositoriesForOrganization(org.login);
          orgRepos.push(...repos);
        } catch (err: any) {
          console.warn(`Failed to load repositories for organization ${org.login}:`, err);
        }
      }
  ...
  let currentRepositoryDisplay = $derived.by(() => {
    ...
    if (currentOrganization && currentRepository.startsWith(currentOrganization + "/")) {
  ...
  let repositoryOptionsWithRecent = $derived.by(() => {
    ...
    const recentRepos = recentlyUsedRepos
      .filter((repo) => {
        if (currentOrganization) {
          return sortedRepositories.some((r) => r.name === repo || r.full_name === repo);
        } else {
    ...
    const otherOptions = allOptions.filter((option) => !recentRepos.includes(option));
  ...
  function addRecentlyUsedOrg(org: string): void {
    ...
    recentlyUsedOrgs = [org, ...recentlyUsedOrgs.filter((o) => o !== org)].slice(0, 5);
  }

  function addRecentlyUsedRepo(repo: string): void {
    if (!repo || recentlyUsedRepos.includes(repo)) return;
    recentlyUsedRepos = [repo, ...recentlyUsedRepos.filter((r) => r !== repo)].slice(0, 5);
  }
  ...
  <FilterButton
    ...
    onselect={(value) =>
      setOrganizationFilter(
        value === "---" || value === "" || value === "Select organization" ? null : value
      )}
...
<div class="secondary-filters-row-1">
  ...
  <button
    class="task-sync-filter-toggle {activeTab === 'pull-requests' ? 'active' : ''}"
