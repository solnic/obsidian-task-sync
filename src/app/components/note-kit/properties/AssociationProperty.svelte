...
<script lang="ts">
  ...
  // Helper to convert entity to wiki link format: [[filepath|DisplayName]]
  // Uses the actual file path from the entity's source to ensure correctness
  function toWikiLink(entityId: string, entityName: string): string {
    const entity = availableEntities.find(e => e.id === entityId);
    const filePath = entity?.filePath;

    if (filePath) {
      // Use the actual file path from the entity
      return `[[${filePath}|${entityName}]]`;
    }

    // Fallback: generate path (should not normally happen for persisted entities)
    const folder = noteTypeId === "task" ? "Tasks" : noteTypeId === "project" ? "Projects" : "Areas";
    return `[[${folder}/${entityName}.md|${entityName}]]`;
  }

  // Helper to extract display name from wiki link format: [[filename.md|DisplayName]]
  function fromWikiLink(wikiLink: string): string | null {
  ...
  const selectedEntityIds = $derived.by(() => {
    ...
    return selectedValues
      .map((val) => {
        // Extract display name from wiki link
        const displayName = fromWikiLink(val);
        if (!displayName) return null;

        // Find entity by name to get its ID
        const entity = availableEntities.find(e => e.name === displayName);
        return entity?.id;
      })
  ...
  const selectedEntityNames = $derived.by(() => {
    ...
    return selectedValues
      .map((val) => {
        // If it's a wiki link, extract the display name
        const displayName = fromWikiLink(val);
  ...
  function removeItem(itemToRemove: string) {
    ...
    const newValues = value.filter(v => {
      const displayName = fromWikiLink(v);
      return displayName !== itemToRemove;
    });
  ...
  function handleSelect(selectedValue: string) {
    ...
    // Convert to wiki link format: [[filepath|DisplayName]]
    const wikiLink = toWikiLink(entity.id, entity.name);
    console.log("[AssociationProperty] Created wiki link:", wikiLink);

    // Handle normal selection
    if (isMultiple) {
      const currentValues = Array.isArray(value) ? value : value ? [value] : [];
      const newValues = currentValues.includes(wikiLink)
        ? currentValues.filter((v) => v !== wikiLink)
        : [...currentValues, wikiLink];
      console.log("[AssociationProperty] Updating multiple values:", newValues);
      value = newValues;
      onvaluechange?.(newValues);
    } else {
      console.log("[AssociationProperty] Updating single value:", wikiLink);
      value = wikiLink;
      onvaluechange?.(wikiLink);
    }
  ...
  async function handleCreateEntity() {
    ...
    try {
      ...
      const wikiLink = toWikiLink(createdEntity.id, entityName);

      // Add the newly created entity to the selection
      if (isMultiple) {
        const currentValues = Array.isArray(value) ? value : value ? [value] : [];
        value = [...currentValues, wikiLink];
        onvaluechange?.(value);
      } else {
        value = wikiLink;
        onvaluechange?.(wikiLink);
