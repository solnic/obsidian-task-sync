<script lang="ts">
  ...
  let previousCalendarIds = $state<string | null>(null);
  ...
  $effect(() => {
    ...
    const currentCalendarIdsStr = JSON.stringify(selectedCalendarIds.sort());

    // Skip if not initialized yet or if nothing changed
    if (!isInitialized) {
    ...
    // Only load events if we have calendars and something actually changed
    if (
