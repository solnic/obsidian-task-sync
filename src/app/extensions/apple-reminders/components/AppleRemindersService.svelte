<script lang="ts">
  ...
  let tasks = $derived.by((): Task[] => {
    // Don't show any tasks if no list is selected
    if (!filters.list) {
      return [];
    }
    ...
    // When filters.completed is FALSE (default), show only incomplete tasks
    // When filters.completed is TRUE, show ALL tasks (including completed)
    ...
    if (filters.priority !== "all") {
      processed = processed.filter((task) => {
        ...
        // Case-insensitive comparison since task.priority might be "High", "Medium", "Low"
        return task.priority?.toLowerCase() === filters.priority.toLowerCase();
      });
    }
  ...
  async function loadInitialData(): Promise<void> {
    ...
    try {
      ...
      if (permissionStatus === "granted") {
        ...
        // The extension's load() method already handles refreshing when there's no cache.
        // The refresh button in the UI will explicitly call refresh() when the user wants to update.
      }
    } catch (err: any) {
...
>
  ...
  <header>
    ...
    <!-- 2. Primary filter buttons group - List selection (full width) -->
    <div class="primary-filters">
      <FilterButton
        label="List"
        currentValue={filters.list || "Select list"}
        options={["Select list", ...availableLists]}
        placeholder="Select list"
        onselect={(value) =>
          setListFilter(value === "Select list" || value === "" ? null : value)}
        testId="list-filter"
        autoSuggest={true}
        allowClear={true}
    ...
    <!-- 3. Secondary filter buttons group - first row: Show completed toggle -->
    <div class="secondary-filters-row-1">
      <button
        class="task-sync-filter-toggle {filters.completed ? 'active' : ''}"
        data-testid="show-completed-filter"
    ...
    <!-- 4. Secondary filter buttons group - second row: Priority filters -->
      ...
      <div class="task-sync-task-list" data-testid="apple-reminders-list">
        {#if !filters.list}
          <div class="task-sync-empty-message">
            Please select a list to view reminders.
          </div>
