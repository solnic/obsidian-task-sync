...
// Entity properties store plain names directly (no wiki link format)
// No helper function needed - values are already plain names
...
export class AssociationCleanup {
  ...
  private cleanupProjectReferences(projectName: string): void {
    ...
    // Entity properties store plain project names (wiki links only in frontmatter)
  ...
  private cleanupAreaReferences(areaName: string): void {
    ...
    // Entity properties store plain area names (wiki links only in frontmatter)
    for (const task of taskState.tasks) {
      const filteredAreas = task.areas.filter((area) => area !== areaName);
    ...
    for (const project of projectState.projects) {
      const filteredAreas = project.areas.filter((area) => area !== areaName);
  ...
  async cleanupOrphanedAssociations(): Promise<{
    ...
    for (const task of taskState.tasks) {
      ...
      // Check project reference (entity properties store plain names)
      if (task.project) {
        if (!validProjectNames.has(task.project)) {
          updatedProject = "";
          needsUpdate = true;
        }
      }

      // Check area references (entity properties store plain names)
