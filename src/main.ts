...
export default class TaskSyncPlugin extends Plugin {
  ...
  private async createTypeNoteWithPersistence(): Promise<TypeNote> {
    ...
    // Ensure default Task note type exists
    await this.ensureDefaultNoteTypes(typeNote);

    return typeNote;
  }

  /**
   * Ensure default note types exist (e.g., Task)
   * This is needed for tests and first-time users
   */
  private async ensureDefaultNoteTypes(typeNote: TypeNote): Promise<void> {
    const existingTypes = typeNote.registry.getAll();

    // Check if Task note type already exists
    const hasTaskType = existingTypes.some((t) => t.id === "task");

    if (!hasTaskType) {
      // Import schemas
      const { stringSchema, optionalStringSchema, booleanSchema } =
        await import("./app/core/type-note/schemas");

      // Create default Task note type
      const taskNoteType = {
        id: "task",
        name: "Task",
        version: "1.0.0",
...
{{Description}}`,
          variables: {
            ...
            Category: { name: "Category", defaultValue: "Task" },
            Status: { name: "Status", defaultValue: "Backlog" },
            Done: { name: "Done", defaultValue: false },
          },
        },
      };

      // Register the default Task note type
      typeNote.registry.register(taskNoteType, {
        allowOverwrite: false,
        validate: true,
      });

      console.log("Created default Task note type");
    }
  }
