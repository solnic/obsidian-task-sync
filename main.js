...
var TaskCreateModal = class extends import_obsidian3.Modal {
  ...
  initializeFormData() {
    this.formData = {
      name: "",
      type: this.plugin.settings.taskTypes[0] || "Task",
  ...
  createFormFields(container) {
    ...
    new import_obsidian3.Setting(container).setName("Type").setDesc("Specify the type of task").addDropdown((dropdown) => {
      ...
      dropdown.setValue(this.formData.type || this.plugin.settings.taskTypes[0]).onChange((value) => {
...
var TaskSyncSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  createTaskTypesSection(container) {
    ...
    let newTypeInput;
    const addSetting = new import_obsidian6.Setting(section).setName("New Task Type").setDesc("Enter a name for the new task type").addText((text) => {
  ...
  renderTaskTypesList(container) {
    const count = this.plugin.settings.taskTypes.length;
    const headerText = count === 0 ? "Task Types" : count === 1 ? "1 Task Type" : `${count} ${(0, import_pluralize.default)("Type", count)}`;
    ...
    this.plugin.settings.taskTypes.forEach((type2, index) => {
      const typeItem = listContainer.createDiv("task-sync-task-type-item");
      const typeLabel = typeItem.createEl("span", { text: type2, cls: "task-sync-task-type-label" });
