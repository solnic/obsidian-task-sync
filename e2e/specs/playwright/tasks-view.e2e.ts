...
test.describe("TasksView Component", () => {
  ...
  test("should filter tasks using search functionality", async ({ page }) => {
    // Create test tasks with different titles
    await createTestTask(page, {
      title: "Search Filter Task",
      category: "Feature",
    });
    ...
    await searchTasks(page, "Search Filter");
    ...
    const taskText = await visibleTasks[0].textContent();
    expect(taskText).toContain("Search Filter Task");
  ...
  test("should open task file when Open button is clicked", async ({
    ...
    const editorContent = await page.evaluate(() => {
      ...
      const activeView = app.workspace.getActiveViewOfType(
        app.viewRegistry.getViewTypeByName("markdown")
      );
      return activeView ? activeView.getViewData() : "";
    });

    expect(editorContent).toContain("This task should open when clicked");
  });
