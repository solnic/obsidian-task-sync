  ...
  test('should provide settings UI to create templates when custom template is configured', async () => {
    ...
    await context.page.evaluate(() => {
      ...
      const taskTemplateSetting = settings.find(setting =>
        setting.querySelector('.setting-item-name')?.textContent?.includes('Default Task Template')
      );

      if (taskTemplateSetting) {
        const input = taskTemplateSetting.querySelector('input[type="text"]') as HTMLInputElement;
        if (input) {
          console.log('Found Default Task Template input, current value:', input.value);
    ...
    await context.page.evaluate(() => {
      ...
      const createButton = buttons.find(button =>
        ...
        button.closest('.setting-item')?.querySelector('.setting-item-name')?.textContent?.includes('Task')
      );
      if (createButton) {
