...
describe('Template Refresh Bug', () => {
  ...
  test('should not corrupt Area and Project templates during refresh', async () => {
    ...
    await context.page.evaluate(async () => {
      ...
      const templateFiles = ['Templates/Area.md', 'Templates/Project.md', 'Templates/Task.md'];
...
{{description}}`;
    ...
    const originalAreaContent = await getFileContent(context.page, 'Templates/Area.md');
    const originalProjectContent = await getFileContent(context.page, 'Templates/Project.md');
    const originalTaskContent = await getFileContent(context.page, 'Templates/Task.md');
    ...
    const afterAreaContent = await getFileContent(context.page, 'Templates/Area.md');
    const afterProjectContent = await getFileContent(context.page, 'Templates/Project.md');
    const afterTaskContent = await getFileContent(context.page, 'Templates/Task.md');
  ...
  test('should create missing templates during refresh', async () => {
    ...
    expect(await fileExists(context.page, 'Templates/MissingArea.md')).toBe(false);
    expect(await fileExists(context.page, 'Templates/MissingProject.md')).toBe(false);
    expect(await fileExists(context.page, 'Templates/MissingTask.md')).toBe(false);
    ...
    expect(await fileExists(context.page, 'Templates/MissingArea.md')).toBe(true);
    expect(await fileExists(context.page, 'Templates/MissingProject.md')).toBe(true);
    expect(await fileExists(context.page, 'Templates/MissingTask.md')).toBe(true);

    // Verify they have correct content structure
    const areaContent = await getFileContent(context.page, 'Templates/MissingArea.md');
    const projectContent = await getFileContent(context.page, 'Templates/MissingProject.md');
    const taskContent = await getFileContent(context.page, 'Templates/MissingTask.md');
