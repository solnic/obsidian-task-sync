...
var TaskSyncPlugin = class extends import_obsidian4.Plugin {
  async onload() {
...
var TaskSyncSettingTab = class extends import_obsidian4.PluginSettingTab {
  ...
  createTabbedInterface(containerEl) {
    ...
    const tabs = [
      ...
      { id: "bases", label: "\u{1F5C2}\uFE0F Bases", content: () => this.createBaseSettings(tabContent) }
    ];
  ...
  createTemplateSettings(container) {
    ...
    new import_obsidian4.Setting(container).setName("Use Templater Plugin").setDesc("Enable integration with Templater plugin for advanced templates").addToggle((toggle) => toggle.setValue(this.plugin.settings.useTemplater).onChange(async (value) => {
      this.plugin.settings.useTemplater = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian4.Setting(container).setName("Default Task Template").setDesc("Default template to use when creating new tasks").addText((text) => text.setPlaceholder("task-template.md").setValue(this.plugin.settings.defaultTaskTemplate).onChange(async (value) => {
      this.plugin.settings.defaultTaskTemplate = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian4.Setting(container).setName("Default Project Template").setDesc("Default template to use when creating new projects").addText((text) => text.setPlaceholder("project-template.md").setValue(this.plugin.settings.defaultProjectTemplate).onChange(async (value) => {
      this.plugin.settings.defaultProjectTemplate = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian4.Setting(container).setName("Default Area Template").setDesc("Default template to use when creating new areas").addText((text) => text.setPlaceholder("area-template.md").setValue(this.plugin.settings.defaultAreaTemplate).onChange(async (value) => {
      this.plugin.settings.defaultAreaTemplate = value;
      await this.plugin.saveSettings();
    }));
  }
  createBaseSettings(container) {
    ...
    new import_obsidian4.Setting(container).setName("Tasks Base File").setDesc("Name of the main tasks base file").addText((text) => text.setPlaceholder("Tasks.base").setValue(this.plugin.settings.tasksBaseFile).onChange(async (value) => {
      this.plugin.settings.tasksBaseFile = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian4.Setting(container).setName("Auto-Generate Bases").setDesc("Automatically create and update base files when the plugin loads").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoGenerateBases).onChange(async (value) => {
      this.plugin.settings.autoGenerateBases = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian4.Setting(container).setName("Auto-Update Base Views").setDesc("Automatically refresh base views when tasks, projects, or areas change").addToggle((toggle) => toggle.setValue(this.plugin.settings.autoUpdateBaseViews).onChange(async (value) => {
