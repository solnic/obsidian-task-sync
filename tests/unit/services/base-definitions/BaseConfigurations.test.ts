...
describe("Base File Generation", () => {
  ...
  describe("Tasks Base YAML Structure", () => {
    ...
    it("should generate base with numbered properties and metadata properties", () => {
      ...
      const createdAtProp = Object.values(parsed.properties).find(
        (p: any) => p.name === "Created At",
      );
      const updatedAtProp = Object.values(parsed.properties).find(
        (p: any) => p.name === "Updated At",
      );
    ...
    it("should generate all required task types and priority views", () => {
      ...
      const featureView = parsed.views.find(
        (v: any) => v.name === "All Features",
      );
      const improvementView = parsed.views.find(
        (v: any) => v.name === "All Improvements",
      );
      ...
      const bugLowView = parsed.views.find(
        (v: any) => v.name === "Bugs • Low priority",
      );
      const featureHighView = parsed.views.find(
        (v: any) => v.name === "Features • High priority",
      );
  ...
  describe("Area Base Generation", () => {
    ...
    it("should include area-specific filters", () => {
      ...
      const tasksView = parsed.views.find((v: any) => v.name === "Tasks");
      expect(tasksView.filters.and).toContain(
        'Areas.contains(link("Task Sync"))',
      );
    });
  });
  ...
  describe("YAML Structure Verification", () => {
    it("should generate YAML structure that matches the user example format", () => {
      ...
      const createdAtKey = Object.keys(parsed.properties).find(
        (key) => parsed.properties[key].name === "Created At",
      );
      const updatedAtKey = Object.keys(parsed.properties).find(
        (key) => parsed.properties[key].name === "Updated At",
      );
