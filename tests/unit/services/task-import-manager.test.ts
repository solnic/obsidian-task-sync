...
describe("TaskImportManager - Pure Logic", () => {
  ...
  const taskImportManager = new TaskImportManager(
    null as any,
    null as any,
    mockSettings,
  );
  ...
  it("should always use configured tasks folder regardless of context", () => {
    ...
    const projectFolder =
      taskImportManager.determineTaskFolder(configWithProject);
  ...
  it("should generate proper front matter for external task", () => {
    ...
    const frontMatter = taskImportManager.generateTaskFrontMatter(
      taskData,
      config,
    );
  ...
  it("should extract priority from external task data", () => {
    ...
    const frontMatter1 = taskImportManager.generateTaskFrontMatter(
      taskDataWithPriority,
      config,
    );
    const frontMatter2 = taskImportManager.generateTaskFrontMatter(
      taskDataWithLabels,
      config,
    );
