...
describe("TaskSync Settings", () => {
  describe("Settings Interface", () => {
    it("should have correct default settings structure", () => {
      ...
      expect(DEFAULT_SETTINGS.defaultProjectTemplate).toBe(
        "project-template.md",
      );
      expect(DEFAULT_SETTINGS.defaultAreaTemplate).toBe("area-template.md");
      expect(DEFAULT_SETTINGS.defaultParentTaskTemplate).toBe(
        "parent-task-template.md",
      );
      ...
      expect(DEFAULT_SETTINGS.taskPropertyOrder).toHaveLength(
        PROPERTY_SETS.TASK_FRONTMATTER.length,
      );
    });
