...
test.describe("Apple Reminders Settings", () => {
  test("should allow selecting reminder lists in settings", async ({ page }) => {
    ...
    const appleRemindersSection = page.locator('h3:has-text("Apple Reminders")');
    await expect(appleRemindersSection).toBeVisible();

    // Enable Apple Reminders integration if not already enabled
    const enableToggle = page.locator('.setting-item:has-text("Enable Apple Reminders Integration") input[type="checkbox"]');
    ...
    const reminderListsSetting = page.locator('.setting-item:has-text("Reminder Lists")');
    ...
    const dropdownVisible = await page.locator('[data-testid="apple-reminders-lists-dropdown"]').isVisible().catch(() => false);
    ...
    const dropdown = page.locator('[data-testid="apple-reminders-lists-dropdown"]');
    await expect(dropdown).toBeVisible({ timeout: 5000 });

    // Verify lists are shown in dropdown
    const workList = dropdown.locator('.task-sync-selector-item:has-text("Work")');
    const personalList = dropdown.locator('.task-sync-selector-item:has-text("Personal")');
