  ...
  test("should reactively update imported issue when task note is modified", async ({
    ...
    await page.waitForFunction(
      ({ url }) => {
        const app = (window as any).app;
        const plugin = app.plugins.plugins["obsidian-task-sync"];
        const task = plugin.stores.taskStore.findBySourceUrl(url);
        if (task) {
    ...
    const updatedTask = await page.evaluate(async () => {
      const app = (window as any).app;
      const plugin = app.plugins.plugins["obsidian-task-sync"];

      const task = await plugin.stores.taskStore.findByTitle(
        "First test issue"
      );
      return task;
    });
    ...
    // Wait a bit for reactivity to propagate the doDate change
    await page.waitForTimeout(1000);
    ...
    const scheduledBadge = issueLocator.locator('[title*="Scheduled"]');
