...
`;
  ...
  test("should detect existing tasks when tasks folder is configured differently", async ({
    page,
  }) => {
    const todayString = getTodayString();

    // Change the tasks folder setting to something other than "Tasks"
    await page.evaluate(async () => {
      const plugin = (window as any).app.plugins.plugins["obsidian-task-sync"];
      if (plugin) {
        plugin.settings.tasksFolder = "MyTasks";
        await plugin.saveSettings();
      }
    });

    // Create tasks for today in the custom folder
    const task1 = await createTask(page, {
      title: "Custom Folder Test Task 1",
      description: "Task to test custom tasks folder",
      status: "Not Started",
      priority: "High",
      done: false,
      doDate: todayString,
    ...
    const task2 = await createTask(page, {
      title: "Custom Folder Test Task 2",
      description: "Another task to test custom tasks folder",
      status: "Not Started",
      priority: "Medium",
      done: false,
      doDate: todayString,
    ...
    // Create daily note with task links using the custom folder path
    const preExistingContent = `# ${todayString}

## Today's Tasks
- [ ] [[MyTasks/Custom Folder Test Task 1|Custom Folder Test Task 1]]
- [ ] [[MyTasks/Custom Folder Test Task 2|Custom Folder Test Task 2]]
...
`;
    ...
    expect(initialContent).toBeTruthy();
    expect(initialContent).toContain("MyTasks/Custom Folder Test Task 1");
    expect(initialContent).toContain("MyTasks/Custom Folder Test Task 2");

    // Count initial task links
    const initialTask1Count = (
      initialContent!.match(
        /^- \[ \] \[\[MyTasks\/Custom Folder Test Task 1\|Custom Folder Test Task 1\]\]$/gm
      ) || []
    ).length;
    const initialTask2Count = (
      initialContent!.match(
        /^- \[ \] \[\[MyTasks\/Custom Folder Test Task 2\|Custom Folder Test Task 2\]\]$/gm
      ) || []
    ).length;
    ...
    // NOW run daily planning - DailyNoteFeature should detect existing tasks despite custom folder
    await executeCommand(page, "Task Sync: Start Daily Planning");
    await expect(
      page.locator('[data-testid="daily-planning-view"]')
    ).toBeVisible({ timeout: 10000 });
    ...
    // Log for debugging
    console.log("Final content for custom folder test:", finalContent);

    // Count task links after confirmation - check for duplication
    const finalTask1Count = (
      finalContent!.match(
        /^- \[ \] \[\[MyTasks\/Custom Folder Test Task 1\|Custom Folder Test Task 1\]\]$/gm
      ) || []
    ...
    console.log("Final task counts:", { finalTask1Count, finalTask2Count });

    // CRITICAL ASSERTION: Should still be 1 per task (no duplication)
    // If the bug exists, DailyNoteFeature won't detect existing tasks because it uses hardcoded "Tasks"
    // instead of the configured "MyTasks" folder, leading to duplication
    expect(finalTask1Count).toBe(1);
    expect(finalTask2Count).toBe(1);
    ...
    await page.evaluate(async () => {
      const plugin = (window as any).app.plugins.plugins["obsidian-task-sync"];
      if (plugin) {
        plugin.settings.tasksFolder = "Tasks";
        await plugin.saveSettings();
      }
    });
  });
