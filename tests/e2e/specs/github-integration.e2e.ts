...
test.describe("GitHub Integration", () => {
  ...
  test("should NOT display pull requests in the issues list", async ({
    ...
    const issueCount = await page
      .locator(
        '[data-testid="service-content-github"]:not(.tab-hidden) .task-sync-item-title'
      )
      .count();
    ...
    expect(
      await page
        .locator(
          "[data-testid=\"service-content-github\"]:not(.tab-hidden) .task-sync-item-title:has-text('First test issue')"
        )
        .count()
    ).toBe(1);
    ...
    expect(
      await page
        .locator(
          "[data-testid=\"service-content-github\"]:not(.tab-hidden) .task-sync-item-title:has-text('Second test issue')"
        )
        .count()
    ).toBe(1);
    ...
    expect(
      await page
        .locator(
          "[data-testid=\"service-content-github\"]:not(.tab-hidden) .task-sync-item-title:has-text('Add new feature')"
        )
        .count()
    ).toBe(0);
    ...
    expect(
      await page
        .locator(
          "[data-testid=\"service-content-github\"]:not(.tab-hidden) .task-sync-item-title:has-text('Fix critical bug')"
        )
        .count()
    ).toBe(0);
  });
  ...
  test("should persist recently used organizations and repositories across plugin reload", async ({
    ...
    await expect(
      page.locator('[data-testid="organization-filter"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="organization-filter"]')
    ).not.toBeDisabled();
