...
test.describe("NoteKit BasesIntegration", () => {
  test("should create Obsidian base from NoteKit note type", async ({
    ...
    await page.locator('[data-testid="note-type-name-input"]').fill("Article");
    ...
    const baseCreationResult = await page.evaluate(async () => {
      ...
      return await plugin.typeNote.basesIntegration.createBaseFromNoteType(
        "article"
      );
    });
  ...
  test("should handle property type mapping for base generation", async ({
    ...
    const key1 =
      (await prop1Input.getAttribute("data-testid"))?.replace(
        "property-name-input-",
        ""
      ) || "";
    await page
    ...
    const key2 =
      (await prop2Input.getAttribute("data-testid"))?.replace(
        "property-name-input-",
        ""
      ) || "";
    await page
    ...
    const key3 =
      (await prop3Input.getAttribute("data-testid"))?.replace(
        "property-name-input-",
        ""
      ) || "";
    await page
    ...
    const key4 =
      (await prop4Input.getAttribute("data-testid"))?.replace(
        "property-name-input-",
        ""
      ) || "";
    await page
